...
TECHNICAL_ANALYSIS_PROMPT = """You are a quantitative analyst writing a comprehensive technical analysis report.
...
- Days Since Crossover: {days_since_crossover}
...
- Middle Band: ${middle_band:.2f}
- Lower Band: ${lower_band:.2f}
- Current Price: ${bb_current_price:.2f}
...
- Value Area High: ${value_area_high:.2f}
- Value Area Low: ${value_area_low:.2f}
- Interpretation: {volume_profile_interpretation}
...
- Confidence: {signal_confidence:.0%}
- Bullish Factors: {bullish_count}
- Bearish Factors: {bearish_count}

---

**Task**: Write a comprehensive technical analysis report (600-800 words) covering:

1. **Trend Analysis**: Evaluate the overall trend using SMAs. Is the stock in an uptrend, downtrend, or consolidation? How significant is the crossover signal (if any)?

2. **Momentum Assessment**: Analyze RSI, MACD, and Stochastic Oscillator. Are all momentum indicators aligned, or are there divergences? Is momentum strengthening or weakening?

3. **Volatility & Price Action**: Use Bollinger Bands to assess volatility and price extremes. Is the stock in normal range, or showing overbought/oversold conditions? What does bandwidth suggest about potential breakouts?

4. **Volume Confirmation**: Does volume confirm the price action? Analyze both volume trends and volume profile. Are we seeing institutional accumulation or distribution at key price levels?

5. **Relative Strength**: How is the stock performing vs its sector and the broader market? Is it showing leadership or weakness?

6. **Entry/Exit Signals**: Synthesize all indicators into actionable insights. What are the key price levels to watch (support/resistance based on volume profile and Bollinger Bands)?

7. **Overall Technical Outlook**: Provide a cohesive assessment of trend, momentum, and risk. What do the technicals collectively suggest about the near-term and medium-term outlook?

**Guidelines**:
- Be objective and data-driven
- Explain what each indicator means in practical terms
- Note any divergences or confirmations between indicators (e.g., "price making new highs but MACD diverging lower")
- Highlight confluence zones where multiple indicators agree (e.g., "POC and lower Bollinger Band both at $145 - strong support")
- Provide actionable insights with specific price levels
- Avoid making explicit buy/sell recommendations, but describe what the technical setup suggests
- Use professional financial analysis language
- If indicators are conflicting, acknowledge the uncertainty and note which indicators are more reliable in the current context
"""
